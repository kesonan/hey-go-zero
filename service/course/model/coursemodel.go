...
type (
	...
	Course struct {
		...
		MaleLimit   int64     `db:"male_limit"`   // 男生限制人数 0-不限
		FemaleLimit int64     `db:"female_limit"` // 女生限制人数 0-不限
...
func (m *defaultCourseModel) Insert(data Course) (sql.Result, error) {
	...
	ret, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Name, data.Description, data.Classify, data.GenderLimit, data.MaleLimit, data.FemaleLimit, data.StartTime, data.Credit)
	}, courseNameKey)
	return ret, err
}
...
func (m *defaultCourseModel) Update(data Course) error {
	...
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Name, data.Description, data.Classify, data.GenderLimit, data.MaleLimit, data.FemaleLimit, data.StartTime, data.Credit, data.Id)
	}, courseIdKey)
	return err
}
